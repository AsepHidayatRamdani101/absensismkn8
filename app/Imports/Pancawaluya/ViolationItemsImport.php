...
class ViolationItemsImport implements ToCollection, WithHeadingRow
{
    ...
    public function collection(Collection $rows): void
    {
        foreach ($rows as $index => $row) {
            ...
            $data = [
                ...
                'code' => trim((string) ($row['kode_pelanggaran'] ?? $row['code'] ?? '')),
                'name' => trim((string) ($row['nama_pelanggaran'] ?? $row['name'] ?? '')),
            ...
            $category = ViolationCategory::query()->where('code', $data['category_code'])->first();
            if (!$category) {
                throw ValidationException::withMessages([
                    'file' => 'Baris ' . ($index + 2) . ': kategori pelanggaran dengan kode ' . $data['category_code'] . ' tidak ditemukan.',
                ]);
            }
            ...
            $item = ViolationItem::query()->withTrashed()->firstOrNew(['code' => $data['code']]);
            $exists = $item->exists;
            ...
            CharacterMapping::query()
                ->where('mappable_type', ViolationItem::class)
                ->where('mappable_id', $item->id)
                ->delete();

            CharacterMapping::query()->create([
                'mappable_type' => ViolationItem::class,
                'mappable_id' => $item->id,
    ...
    {
        return "Import pelanggaran selesai. Dibuat {$this->createdCount}, diperbarui {$this->updatedCount}.";
    }
