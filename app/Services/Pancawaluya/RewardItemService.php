...
class RewardItemService
{
    public function __construct(
        private readonly RewardItemRepositoryInterface $repository,
    ...
    public function findForEdit(int $id, bool $withTrashed = false): ?RewardItem
    {
        return $this->repository->findById($id, $withTrashed);
    }

    public function create(array $data, Request $request): RewardItem
    {
        return DB::transaction(function () use ($data, $request): RewardItem {
            ...
            $this->mappingRepository->syncForRewardItem($item->id, $dimensionId, $weight);

            $this->auditTrailService->log(RewardItem::class, $item->id, 'CREATE', null, $item->toArray(), $request, 'WEB');

            return $item->refresh()->load(['category', 'mappings.dimension']);
        });
    }

    public function update(RewardItem $item, array $data, Request $request): RewardItem
    {
        return DB::transaction(function () use ($item, $data, $request): RewardItem {
            $this->assertDimensionExists((int) $data['character_dimension_id']);
            ...
            $this->mappingRepository->syncForRewardItem($updated->id, $dimensionId, $weight);

            $updated = $updated->refresh()->load(['category', 'mappings.dimension']);

            $this->auditTrailService->log(RewardItem::class, $updated->id, 'UPDATE', $before, $updated->toArray(), $request, 'WEB');

            return $updated;
        });
    }

    public function softDelete(RewardItem $item, Request $request): void
    {
        DB::transaction(function () use ($item, $request): void {
            ...
            $this->auditTrailService->log(RewardItem::class, $item->id, 'DELETE', $before, null, $request, 'WEB');
        });
    }
    ...
    public function restore(int $id, Request $request): bool
    {
        return DB::transaction(function () use ($id, $request): bool {
            ...
            if ($restored) {
                $this->auditTrailService->log(RewardItem::class, $id, 'RESTORE', null, ['id' => $id], $request, 'WEB');
            }
    ...
    public function forceDelete(int $id, Request $request): bool
    {
        return DB::transaction(function () use ($id, $request): bool {
            ...
            if ($deleted) {
                $this->auditTrailService->log(RewardItem::class, $id, 'FORCE_DELETE', null, null, $request, 'WEB');
            }
    ...
    public function bulkSoftDelete(array $ids, Request $request): int
    {
        return DB::transaction(function () use ($ids, $request): int {
            ...
            $this->auditTrailService->log(RewardItem::class, null, 'BULK_DELETE', ['ids' => $ids], ['count' => $count], $request, 'WEB');
    ...
    public function bulkRestore(array $ids, Request $request): int
    {
        return DB::transaction(function () use ($ids, $request): int {
            ...
            $this->auditTrailService->log(RewardItem::class, null, 'BULK_RESTORE', ['ids' => $ids], ['count' => $count], $request, 'WEB');
