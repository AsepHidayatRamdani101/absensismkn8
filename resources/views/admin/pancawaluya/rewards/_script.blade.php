<script>
    $(function() {
        ...
        const table = $('#tableRewards').DataTable({
            processing: true,
            serverSide: true,
            responsive: true,
            ajax: {
                url: "{{ route('pancawaluya.rewards.datatable') }}",
                data: function(d) {
        ...
        $(document).on('click', '.btn-restore', function() {
            const id = $(this).data('id');
            $.post("{{ url('pancawaluya/rewards') }}/" + id + "/restore", {
                _token: '{{ csrf_token() }}'
            }, function(resp) {
                Swal.fire('Berhasil', resp.message, 'success');
                table.ajax.reload();
        ...
        $(document).on('click', '.btn-force-delete', function() {
            ...
            Swal.fire({
                ...
                if (!result.isConfirmed) return;
                $.ajax({
                    url: "{{ url('pancawaluya/rewards') }}/" + id + "/force-delete",
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        _method: 'DELETE'
        ...
        $('#btnDeleteSelected').on('click', function() {
            ...
            $.ajax({
                url: "{{ route('pancawaluya.rewards.bulk-delete') }}",
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    _method: 'DELETE',
        ...
        $('#btnRestoreSelected').on('click', function() {
            ...
            if (!ids.length) return;
            $.post("{{ route('pancawaluya.rewards.bulk-restore') }}", {
                _token: '{{ csrf_token() }}',
                ids: ids
            }, function(resp) {
                Swal.fire('Berhasil', resp.message, 'success');
