...
@section('title', 'Laporan Izin Siswa')

@section('plugins.Datatables', true)

@section('content_header')
    <div class="d-flex justify-content-between align-items-center">
        <h1>Laporan Izin Siswa</h1>
        <div class="d-flex">
            <a href="{{ route('reports.student-leave.pdf', request()->query()) }}" class="btn btn-danger btn-sm mr-2"><i
                    class="fas fa-file-pdf"></i> Download PDF</a>
            <a href="{{ route('reports.student-leave.excel', request()->query()) }}" class="btn btn-success btn-sm"><i
...
@section('content')
    <div class="card">
        <div class="card-body">
            <form method="GET" action="{{ route('reports.student-leave') }}" class="mb-3">
                <div class="form-row">
                    ...
                    <div class="form-group col-md-3"><label>Nama Siswa</label><select name="student_id"
                            class="form-control form-control-sm">
                    ...
                    <div class="form-group col-md-3"><label>Kelas</label><select name="classroom_id"
                            class="form-control form-control-sm">
                            <option value="">Semua Kelas</option>
                            @foreach ($classrooms as $classroom)
                                <option value="{{ $classroom->id }}"
                                    {{ (string) ($filters['classroom_id'] ?? '') === (string) $classroom->id ? 'selected' : '' }}>
                                    {{ $classroom->nama_kelas }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group col-md-3 d-flex align-items-end"><button type="submit"
                            ...
                            href="{{ route('reports.student-leave') }}" class="btn btn-secondary btn-sm">Reset</a></div>
                </div>
            </form>
            <p class="text-muted mb-2"><strong>Periode:</strong> {{ $periodLabel }}</p>
            <div class="table-responsive">
                <table id="tableStudentLeaveReport" class="table table-bordered table-striped">
                    <thead>
                        <tr>
                            <th>No</th>
                            <th>Siswa</th>
...
@section('js')
    <script>
        $(function() {
            $('#tableStudentLeaveReport').DataTable({
                responsive: true,
                language: {
