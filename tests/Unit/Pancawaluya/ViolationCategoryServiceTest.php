...
class ViolationCategoryServiceTest extends TestCase
{
    public function test_paginate_delegates_to_repository(): void
    {
        $repository = \Mockery::mock(ViolationCategoryRepositoryInterface::class);
        $paginator = new LengthAwarePaginator([], 0, 10, 1);
        $repository->shouldReceive('paginate')->once()->with(['search' => 'abc'], 10)->andReturn($paginator);

        $audit = \Mockery::mock(AuditTrailService::class);

        $service = new ViolationCategoryService($repository, $audit);
    ...
    public function test_restore_calls_repository_restore(): void
    {
        ...
        $repository = \Mockery::mock(ViolationCategoryRepositoryInterface::class);
        $repository->shouldReceive('restore')->once()->with(11)->andReturn(true);
        ...
        $result = $service->restore(11, Request::create('/'));
